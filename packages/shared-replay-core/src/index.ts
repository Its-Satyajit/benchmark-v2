...
export function simulateReplay(
		...
		for (let j = 0; j < stepBatch.length; j++) {
			...
			if (step.observation?.current?.players) {
				...
				for (let p = 0; p < players.length; p++) {
					...
					totalCardsObserved +=
						(player.deck?.length ?? 0) + (player.hand?.length ?? 0);
					hasher.update(
	...
	hasher.update(
		`final:steps=${stepsProcessed}:cards=${totalCardsObserved}:acts=${actionTransitions}`,
	);
...
export function simulateStressReplay(
	...
	hasher.update(
		`final_stress:steps=${totalSteps}:cards=${totalCards}:acts=${totalActs}`,
	);
	const checksum = hasher.digest("hex");
	const endTime = performance.now();
