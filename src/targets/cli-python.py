import sys
import json
import time
import hashlib

def simulate():
    replay_path = None
    is_stress = '--stress' in sys.argv
    iterations = 20
    for i, arg in enumerate(sys.argv):
        if arg == '--replay' and i + 1 < len(sys.argv):
            replay_path = sys.argv[i + 1]
        elif arg == '--iterations' and i + 1 < len(sys.argv):
            try:
                iterations = int(sys.argv[i + 1])
            except ValueError:
                iterations = 20

    if not replay_path:
        print("Error: Missing --replay <path>", file=sys.stderr)
        sys.exit(1)

    parse_start = time.perf_counter()
    with open(replay_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    parse_duration_ms = (time.perf_counter() - parse_start) * 1000.0

    replay_start = time.perf_counter()
    hasher = hashlib.sha256()

    steps = data.get('steps', [])
    total_steps = 0
    total_cards = 0
    total_acts = 0
    latencies = []
    snapshot_tree = []

    actual_iters = iterations if is_stress else 1

    for it in range(actual_iters):
        if is_stress:
            hasher.update(f"iter:{it};".encode('utf-8'))
        cfg = data.get('configuration')
        if cfg and 'seed' in cfg and cfg['seed'] is not None:
            hasher.update(f"seed:{cfg['seed']};".encode('utf-8'))

        for i, step_batch in enumerate(steps):
            step_start = time.perf_counter()
            total_steps += 1
            for j, step in enumerate(step_batch):
                action = step.get('action')
                if action:
                    total_acts += len(action)
                    act_str = json.dumps(action, separators=(',', ':'))
                    if is_stress:
                        hasher.update(f"act:{it}:{i}:{j}:{act_str};".encode('utf-8'))
                    else:
                        hasher.update(f"act:{i}:{j}:{act_str};".encode('utf-8'))

                obs = step.get('observation')
                if obs and isinstance(obs, dict):
                    current = obs.get('current')
                    if current and isinstance(current, dict):
                        players = current.get('players', [])
                        if is_stress:
                            # retain snapshot trees under stress
                            snapshot_tree.append(players)
                        for p, player in enumerate(players):
                            deck_list = player.get('deck') or []
                            hand_list = player.get('hand') or []
                            active_list = player.get('active') or []
                            bench_list = player.get('bench') or []
                            deck_len = len(deck_list)
                            hand_len = len(hand_list)
                            active_len = len(active_list)
                            bench_len = len(bench_list)
                            total_cards += deck_len + hand_len
                            hasher.update(f"p:{p}:d{deck_len}:h{hand_len}:a{active_len}:b{bench_len};".encode('utf-8'))

                status = step.get('status')
                if status:
                    hasher.update(f"st:{status};".encode('utf-8'))

            latencies.append((time.perf_counter() - step_start) * 1000.0)

    if is_stress:
        hasher.update(f"final_stress:steps={total_steps}:cards={total_cards}:acts={total_acts}".encode('utf-8'))
    else:
        hasher.update(f"final:steps={total_steps}:cards={total_cards}:acts={total_acts}".encode('utf-8'))
    checksum = hasher.hexdigest()

    replay_duration_ms = (time.perf_counter() - replay_start) * 1000.0
    total_duration_ms = parse_duration_ms + replay_duration_ms
    steps_per_sec = (total_steps / (replay_duration_ms / 1000.0)) if replay_duration_ms > 0 else 0.0

    latencies.sort()
    p50 = latencies[int(len(latencies) * 0.5)] if latencies else 0.0
    p95 = latencies[int(len(latencies) * 0.95)] if latencies else 0.0
    p99 = latencies[int(len(latencies) * 0.99)] if latencies else 0.0

    output = {
        "target": "python-cli",
        "steps_processed": total_steps,
        "parse_duration_ms": round(parse_duration_ms, 2),
        "replay_duration_ms": round(replay_duration_ms, 2),
        "total_duration_ms": round(total_duration_ms, 2),
        "steps_per_sec": round(steps_per_sec, 2),
        "checksum": checksum,
        "snapshots_retained": len(snapshot_tree) if is_stress else None,
        "p50_latency_ms": round(p50, 3) if is_stress else None,
        "p95_latency_ms": round(p95, 3) if is_stress else None,
        "p99_latency_ms": round(p99, 3) if is_stress else None,
    }
    print(json.dumps(output))

if __name__ == '__main__':
    simulate()
